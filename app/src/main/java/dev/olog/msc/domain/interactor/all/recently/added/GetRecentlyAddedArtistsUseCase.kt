...
class GetRecentlyAddedArtistsUseCase @Inject constructor(
    ...
    override fun buildUseCaseObservable(): Observable<List<Artist>> {
        return Observables.combineLatest(getRecentlyAddedSong(getAllSongsUseCase), getAllArtistsUseCase.execute())
        { songs, artists -> artists.filter { artist -> songs.any { song -> song.artistId == artist.id } } }
    }
}