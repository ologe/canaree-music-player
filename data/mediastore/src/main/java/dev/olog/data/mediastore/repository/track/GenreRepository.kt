...
import dev.olog.data.mediastore.mapper.toArtist
import dev.olog.data.mediastore.mapper.toGenre
import dev.olog.data.mediastore.mapper.toPlaylistTrack
import dev.olog.data.mediastore.queries.GenreQueries
...
import dev.olog.data.mediastore.utils.queryCountRow
...
internal class GenreRepository @Inject constructor(
    ...
    override suspend fun getTrackListByParam(param: Id): List<Track> {
        ...
        return contentResolver.queryAll(cursor) { it.toPlaylistTrack(param) }
    }
    ...
    override fun observeRecentlyAdded(param: Id): Flow<List<Track>> {
        ...
        return observeByParamInternal(contentUri) {
            ...
            contentResolver.queryAll(cursor) { it.toPlaylistTrack(param) }
        }
    }
