...
class MusicPreferencesImpl @Inject constructor(
    ...
    override fun isMidnightMode(): Flow<Boolean> {
        val key = context.getString(prefs.R.string.prefs_midnight_mode_key)
    ...
    private fun setMidnightMode(enable: Boolean){
        preferences.edit {
            putBoolean(context.getString(prefs.R.string.prefs_midnight_mode_key), enable)
        }
    }

    private fun setCrossFade(value: Int){
        val key = context.getString(prefs.R.string.prefs_cross_fade_key)
        preferences.edit { putInt(key, value) }
    }

    override fun observeCrossFade(): Flow<Int> {
        return preferences.observeKey(context.getString(prefs.R.string.prefs_cross_fade_key), 0)
                .map { it * 1000 }
    }

    override fun observeGapless(): Flow<Boolean> {
        return preferences.observeKey(context.getString(prefs.R.string.prefs_gapless_key), false)
    }

    private fun setGapless(enabled: Boolean){
        val key = context.getString(prefs.R.string.prefs_gapless_key)
    ...
    override fun observeShowLockscreenArtwork(): Flow<Boolean> {
        return preferences.observeKey(context.getString(prefs.R.string.prefs_lockscreen_artwork_key), false)
            .flowOn(Dispatchers.IO)
    }
}