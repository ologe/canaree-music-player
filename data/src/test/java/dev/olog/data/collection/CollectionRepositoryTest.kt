...
class CollectionRepositoryTest {
    ...
    private val repo = mock<MediaStoreAlbumRepository>()
    private val podcastRepo = mock<MediaStorePodcastCollectionRepository>()
    private val sut = CollectionRepository(
    ...
    companion object {
        ...
        private val item = MediaStoreAlbum("3")
        ...
        private val detailSort = Sort(
            CollectionDetailSort.Title,
            SortDirection.ASCENDING,
        )

        private val authorUri = MediaUri(
            source = MediaUri.Source.MediaStore,
            category = MediaUri.Category.Author,
            id = "20",
            isPodcast = false
        )
        private val authorPodcastUri = MediaUri(
            source = MediaUri.Source.MediaStore,
            category = MediaUri.Category.Author,
            id = "21",
            isPodcast = true
    ...
    @Test
    fun `test observeArtistsAlbums song`() = runTest {
        whenever(repo.observeArtistsAlbums(authorUri.id)).thenReturn(flowOf(list))
        sut.observeArtistsAlbums(authorUri).test(this) {
            assertValue(list)
        }
    }

    @Test
    fun `test observeArtistsAlbums podcast`() = runTest {
        whenever(podcastRepo.observeArtistsAlbums(authorPodcastUri.id)).thenReturn(flowOf(list))
        sut.observeArtistsAlbums(authorPodcastUri).test(this) {
            assertValue(list)
        }
    }

    @Test
    fun `test observeSiblingsById song`() = runTest {
        whenever(repo.observeSiblingsById(uri.id)).thenReturn(flowOf(list))
        sut.observeSiblingsById(uri).test(this) {
            assertValue(list)
        }
    }

    @Test
    fun `test observeSiblingsById podcast`() = runTest {
        whenever(podcastRepo.observeSiblingsById(podcastUri.id)).thenReturn(flowOf(list))
        sut.observeSiblingsById(podcastUri).test(this) {
            assertValue(list)
        }
    }
