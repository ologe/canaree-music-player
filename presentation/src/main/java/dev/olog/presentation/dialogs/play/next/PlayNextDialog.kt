...
class PlayNextDialog : BaseDialog() {
    ...
    private fun successMessage(context: Context): String = when (mediaId.category) {
        MediaIdCategory.SONGS -> context.getString(R.string.song_x_added_to_play_next, title)
    ...
    private fun createMessage() : String = when (mediaId.category) {
        MediaIdCategory.SONGS -> getString(R.string.add_song_x_to_play_next, title)
        else -> resources.getQuantityString(R.plurals.add_xx_songs_to_play_next, listSize, listSize)
    }
